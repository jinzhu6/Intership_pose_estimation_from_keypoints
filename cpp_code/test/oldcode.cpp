...
gsl_matrix* testPoseEstimation(double *A, double *B, double *D, integer n, integer p, double *X, double *Xopt, bool verb)
{
	...
	RTRSD *solver = new RTRSD(&Prob, &StieX); // TODO : tester plusieur solver pour trouver le meilleur
...
pose pose_from_kps_WP(gsl_matrix* W, dictonnaire dict, gsl_vector* weight, bool verb, int lam_val, double tol)
{
    ...
    const int nb_joint = dict.nb_joints; // number of joints of the model
    const gsl_matrix* pc = dict.pc;
    gsl_matrix* B = gsl_matrix_calloc(m,nb_joint);

    for(int i = 0 ; i < m ; i++)
    {
        for(int j = 0 ; j < nb_joint ; j++)
        {
            gsl_matrix_set(B, i, j, gsl_matrix_get(dict.mu, i, j));
        }
    }

    const int k = (B->size1)/3, alpha = 1;

    gsl_matrix* D = gsl_matrix_calloc(nb_joint, nb_joint);
    gsl_matrix* lam = gsl_matrix_calloc(nb_joint, nb_joint);
    double sum_D = 0;

    for(int i = 0 ; i < nb_joint ; i++)
    {
        gsl_matrix_set(lam, i, i, lam_val); // lam = diag(lam_val)
        gsl_matrix_set(D, i, i, gsl_vector_get(weight,i)); // D = diag(weight)
        sum_D += gsl_vector_get(weight,i); // sum_D = sum(weight)
    }
    ...
    for(int i = 0 ; i < m ; i++)
    {
        for (int j = 0 ; j < nb_joint ; ++j)
        {
            tab_mean[i] += gsl_matrix_get(B,i,j) / nb_joint;
        }
    }

    // centralization of the basis
    for(int i = 0 ; i < m ; i++)
    {
        for (int j = 0 ; j < nb_joint ; ++j)
        {
            gsl_matrix_set(B, i, j, gsl_matrix_get(B,i,j) - tab_mean[i]);
        }
    }
    ...
    if (pc == nullptr)
    {
        gsl_matrix* Cnew = nullptr;
    }
    else
    {
        gsl_matrix* Cnew = gsl_matrix_calloc(1, (pc->size1)/3);
    }
    ...
    for (int i = 0 ; i < 1 ; i++)
    {
        ...
        gsl_matrix* Rnew = estimateR_weighted(S, W2fit, D, R, verb);
        gsl_matrix_memcpy(R,Rnew); // update of R

        if (C == nullptr)
        {
            double C0 = estimateC_weighted(W2fit, R, B, D, 1e-3);
        ...
        double fvaltm1 = fval; // fval = 0.5*norm((W2fit-R*S)*sqrt(D),'fro')^2 + 0.5*norm(C)^2;
        ...
        fval = 0.5*pow(matrix_norm(tmp9),2);
        printf("C\n");
        print_matrix(C);
...
int find_maximal_response(int nb_joints)
{
    ...
    pose_from_kps_WP(W_hp, dict, weight, true, 1, 1e-10);
